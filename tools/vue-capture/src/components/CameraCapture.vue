...
const shape = ref<ThumbnailShape>('circle')
...
const captureBlob = async (): Promise<Blob> => {
  ...
  // Object-fit: cover mapping from visible container coords back to source frame coords.
...
const captureAndSave = async () => {
  ...
  try {
    ...
    await saveThumbnail(blob, shape.value)
    emit('saved')
  } catch (error) {
...
<template>
  <section class="camera-capture">
    <div class="controls-row">
      ...
      <template v-else>
        <button type="button" :disabled="isSaving" @click="captureAndSave">Capture {{ shapeLabel }}</button>
        <button type="button" :disabled="isSaving" @click="toggleShape">Use {{ shape === 'circle' ? 'Square' : 'Circle' }}</button>
        <button type="button" :disabled="isSaving" @click="stopStream">Stop Camera</button>
      </template>
    </div>
